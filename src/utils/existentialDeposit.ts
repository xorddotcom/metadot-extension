...
const MAX = FixedPointNumber.fromInner(
    '0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff'
);
const { ZERO } = FixedPointNumber;
...
const normalizeNetwokrName = (name: string): string => name.toLowerCase();
const normalizeCurrencyName = (name: string): string => name.toUpperCase();

// get ed config, return 0 if the config doesn't set.
export const getExistentialDepositConfig = (
    network: string,
    currency: string
): FixedPointNumber => {
    const config =
        EXISTENTIAL_DEPOSIT?.[normalizeNetwokrName(network)] ||
        EXISTENTIAL_DEPOSIT.dev;
