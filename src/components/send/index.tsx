...
const Send: React.FunctionComponent = () => {
  ...
  const validateTxErrors = async (txFee: number): Promise<void> => {
    try {
      ...
      if (
        ...
        setSubTextForWarningModal(
          `The ${
            chainTokens.length > 1 ? 'sending token' : 'sender account'
          } might get reaped`,
        );
        setIsWarningModalOpen(true);
      }
  ...
  const dispatchAndRedirectAfterTransaction = (callback: () => void): void => {
    setLoading2(false);
  ...
  const doTransactionCallBackForFail = (
    ...
    const transactionRecord = [
      {
        accountFrom: addressMapper(address, api.registry.chainSS58 as number),
        accountTo: [
          addressMapper(receiverAddress, api.registry.chainSS58 as number),
        ],
        amount: [amount],
  ...
  const normalChecks =
    loading1 || isInputEmpty || receiverAddress.length === 0 || toAddressError;
  ...
  return (
    <Wrapper width="89%">
      ...
      <HorizontalContentDiv justifyContent="flex-end" marginTop="28px">
        <SubHeading onClick={handleBatchSwitch}>Batch Transactions</SubHeading>
